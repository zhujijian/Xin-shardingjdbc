 ...
 */
@Slf4j
@ConsumerType(value = ConsumerTypeEnum.BIDEND)
public class BidEndServiceImpl implements ConsumerService {
	...
	public Boolean consumerMessage(String prjNo) {
		log.info("redis延时消费  修改竞价单为待定价开始 ---->");
		try {

			return true;
		} catch (Exception e) {
			log.error("redis延时消费  修改竞价单为待定价异常-->" , e);
		}
